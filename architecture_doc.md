...
* **Security & Sanitization**: Before writing to Chroma, raw summaries pass through an automated sanitizer that redacts SSH private keys (`-----BEGIN PRIVATE KEY-----`) and masks sensitive credentials (e.g. `password=[REDACTED]`) to prevent credential leakage in vectors.
...
4. **Context Injection & Hallucination Guard**:
   ...
   * If a historical range query returns empty, the composer inserts a warning to the LLM: `No stored historical records matched <date_range>. Do not invent activity for this date range.` preventing the model from hallucinating past operations.
